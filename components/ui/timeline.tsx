...
const TimelineItem = ({ item }: { item: TimelineEntry }) => {
  ...
  return (
    ...
    >
      {/* Columna izquierda: punto + año (sticky) */}
...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  // Medimos la altura del bloque para que el "track" tenga la misma altura
  useLayoutEffect(() => {
    const measure = () => {
  ...
  // Progreso de scroll sobre el timeline
  const { scrollYProgress } = useScroll({
    target: containerRef,
    offset: ["start 10%", "end 90%"],
  });

  // Convertimos 0→1 en 0→altura + opacidad
  const heightTransform = useTransform(scrollYProgress, [0, 1], [0, height]);
  const opacityTransform = useTransform(scrollYProgress, [0, 0.1], [0, 1]);

  return (
    <div ref={containerRef} className="w-full bg-transparent font-sans md:px-10">
      {/* padding lateral "falso" */}
